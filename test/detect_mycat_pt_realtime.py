...
def capture_loop(picam, buf):
    ...
    while True:
        ...
        if results.boxes:
            # confidence 가장 높은 박스 1개만 표시
            best = max(results.boxes, key=lambda b: float(b.conf[0]))
            x1, y1, x2, y2 = map(int, best.xyxy[0].tolist())
            conf = float(best.conf[0])
            cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
            print(f"[CAT] conf={conf:.4f}  center=({cx},{cy})")
            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.circle(frame, (cx, cy), 6, (0, 0, 255), -1)
            cv2.putText(frame, f"russian_blue {conf:.3f}",
                        (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        else:
            print(f"[ ---- ] 러시안블루 없음")

        cv2.putText(frame, f"FPS: {fps:.1f}  [MyCAT-PT]",
